...
cidiff.sc <- matrix(0, nrow = length(p0), ncol = length(n0))
cidiff.wl <- matrix(0, nrow = length(p0), ncol = length(n0))
...
for (i in 1:length(p0)) {
  for (j in 1:length(n0)) {
   ...
   cidiff.sc[i,j] <- apply(cidiff, 2, mean)[2]
   cidiff.wl[i,j] <- apply(cidiff, 2, mean)[3]
  } 
}
...
annotate_figure(g.alpha1, top = text_grob("Coversage Under Sample Size (alpha = 0.05)", face = "bold", size = 20))
...
g.alpha2 <- ggarrange(f1,f2,f3,f4,f5, nrow = 3, ncol = 2,
                      labels = c("fig1", "fig2", "fig3", "fig4", "fig5"))
annotate_figure(g.alpha2, top = text_grob("Coversage Under Sample Size (alpha = 0.01)", face = "bold", size = 20))


############################## numerical analysis ##############################
ci.aly <- round(apply(cidiff.sc, 2, mean),3)
cov.aly <- round(apply(coverage.sc, 2, mean)/1000,3)
print(ci.aly)
print(cov.aly)
